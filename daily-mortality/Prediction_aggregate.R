...
library(rgdal)
library(here)
get_prediction_daily <- function(start_year=1996,
                     end_year=2012,
                     cityname,
                     excludeStnIndex,
                     chainNumber=4
...
data_all$station <- as.numeric(factor(data_all$station))
...
#                       end_year=2012,
#                       cityname="Quebec",
#                       excludeStnIndex=NULL # the sixth is only used for fitting, rather than prediction, it is outside ottawa.
#                       ,chainNumber = 2
# )
