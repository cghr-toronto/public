...
obtain_nice_data <- function(input_model, pollutant, xlim=c(0.92,1.06),nationalLine)
{
  
  est <- exp(mapply(function(x){x$coefficients[1:3]}, input_model)*c(8.3,14,14.4))
  
  se <- mapply(function(x){diag(x$var)[1:3]}, input_model)
  
  lower <- exp(mapply(function(x){x$coefficients[1:3]- 1.96*(sqrt(diag(x$var)[1:3]))
  }, input_model)*c(8.3,14,14.4))
...
national_mort <- exp(apply(combine_result_mort$betaBar,2,median)*c(8.3,14,14.4))
national_morb <- exp(apply(combine_result_morb$betaBar,2,median)*c(8.3,14,14.4))
